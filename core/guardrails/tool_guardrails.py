...
from core.guardrails.json_utils import safe_json_loads
from core.guardrails.tool_result_classification import file_mutation_result_landed, tool_result_failed
...
def classify_tool_failure(tool_name: str, result: Any) -> tuple[bool, str]:
    ...
    if isinstance(result, dict):
        if tool_result_failed(tool_name, result):
            return True, " [error]"
        return False, ""
    ...
    if isinstance(result, str) and tool_result_failed(tool_name, result):
        return True, " [error]"
...
def append_toolguard_guidance(result: Any, decision: ToolGuardrailDecision) -> Any:
    ...
    if isinstance(result, dict):
        updated = dict(result)
        note = updated.get("guardrail_note", "")
        updated["guardrail_note"] = (note + suffix).strip()
        return updated
...
def _tool_failure_recovery_hint(tool_name: str, count: int) -> str:
    common = (
        f"{tool_name} has failed {count} times this turn. Inspect the latest error before retrying. "
    )
    if tool_name.startswith("os_"):
        return common + "Verify the target app/window is available or try a different OS action."
...
def _result_hash(result: Any) -> str:
    ...
    if parsed is not None:
        ...
        canonical = str(result) if result is not None else ""
    return _sha256(canonical)
