#!/usr/bin/env bash
# GitHub environment detection helper for VoiceOS skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _voiceos_env="${VOICEOS_HOME:-$HOME/.voiceos}/.env"; [ -f "$_voiceos_env" ] && grep -q "^GITHUB_TOKEN=" "$_voiceos_env" 2>/dev/null; then
