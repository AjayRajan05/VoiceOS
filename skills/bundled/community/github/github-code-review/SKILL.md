...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _voiceos_env="${VOICEOS_HOME:-$HOME/.voiceos}/.env"; [ -f "$_voiceos_env" ] && grep -q "^GITHUB_TOKEN=" "$_voiceos_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_voiceos_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${VOICEOS_HOME:-$HOME/.voiceos}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by VoiceOS. Code looks clean — good test coverage, no security concerns."
