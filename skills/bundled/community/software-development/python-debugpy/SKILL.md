...
metadata:
  voiceos:
    ...
    related_skills: [systematic-debugging, node-inspect-debugger, voiceos-platform]
---
...
- A long-running process (VoiceOS gateway, tui_gateway) misbehaves and you can't restart it
...
The VoiceOS test runner and pytest both support this:
...
For long-lived processes: VoiceOS gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source /home/bb/voiceos-platform/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside VoiceOS you have two practical options:
...
```json
{
  "name": "Attach to VoiceOS",
  "type": "debugpy",
  "request": "attach",
  "connect": { "host": "127.0.0.1", "port": 5678 },
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "/home/bb/voiceos-platform" }
  ]
}
```
...
## Debugging VoiceOS-specific Processes

### Tests
See Recipe 3. Always add `-p no:xdist` or run single tests without xdist.

### `run_agent.py` / CLI — one-shot
Easiest: add `breakpoint()` near the suspect line, then run `python main.py` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `python main.py --tui`)
...
debugpy.wait_for_client()
```
Start `python main.py --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For VoiceOS subagents, debug one process at a time.
