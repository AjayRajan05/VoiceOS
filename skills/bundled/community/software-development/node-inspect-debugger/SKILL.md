...
metadata:
  voiceos:
    ...
    related_skills: [systematic-debugging, python-debugpy, voiceos-platform]
---
...
VoiceOS-specific note: `chrome-remote-interface` is NOT in `ui-tui/package.json`. Install it to a throwaway location if you don't want to dirty the project:
...
```bash
cd /home/bb/voiceos-platform/ui-tui
...
### Debugging a running `python main.py --tui`
...
6. **Running `node inspect` through an agent terminal.** It's a PTY-friendly REPL. In VoiceOS, launch it with `terminal(pty=true)` or `background=true` + `process(action='submit', data='...')`. Non-PTY foreground mode will work for one-shot commands but not for interactive stepping.
