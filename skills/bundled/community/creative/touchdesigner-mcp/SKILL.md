...
metadata:
  voiceos:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, ascii-video]
...
```
VoiceOS -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${VOICEOS_HOME:-$HOME/.voiceos}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to VoiceOS config (if missing)
...
3. **Restart VoiceOS session** to pick up the new MCP server
