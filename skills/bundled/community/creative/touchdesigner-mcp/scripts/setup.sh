...
VOICEOS_HOME_DIR="${VOICEOS_HOME:-$HOME/.voiceos}"
VOICEOS_CFG="${VOICEOS_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure VoiceOS config has twozero_td MCP entry ──
if [[ ! -f "$VOICEOS_CFG" ]]; then
    echo -e " ${FAIL} VoiceOS config not found at ${VOICEOS_CFG}"
    manual_steps+=("Create ${VOICEOS_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$VOICEOS_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in VoiceOS config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to VoiceOS config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$VOICEOS_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${VOICEOS_CFG} manually"); }
    manual_steps+=("Restart VoiceOS session to pick up config change")
fi
