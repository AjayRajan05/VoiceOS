...
## 5. VoiceOS Configuration

### Config location

`$VOICEOS_HOME/config.yaml` (defaults to `~/.voiceos/config.yaml` when `VOICEOS_HOME` is unset)
...
Restart the VoiceOS session for changes to take effect. The MCP connection is
