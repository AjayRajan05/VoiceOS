...
| Change | Upstream | VoiceOS |
|--------|----------|--------|
| Metadata namespace | `openclaw` | `voiceos` |
| Trigger | `/baoyu-infographic` slash command | Natural language skill matching |
| User config | EXTEND.md file (project/user/XDG paths) | Removed — not part of VoiceOS infra |
...
| File operations | Bash commands | VoiceOS file tools (write_file, read_file) |
...
Reference files can be overwritten directly (they're unchanged from upstream). SKILL.md must be manually merged since it contains VoiceOS-specific adaptations.
