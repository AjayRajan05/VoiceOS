...
description: "Operate the Teams meeting summary pipeline via VoiceOS CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions."
version: 1.1.0
...
Everything operator-facing is a `python main.py teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${VOICEOS_HOME:-~/.voiceos}/.env`:
...
```bash
python main.py teams-pipeline validate              # config snapshot — run first after any change
python main.py teams-pipeline token-health          # Graph token status
python main.py teams-pipeline token-health --force-refresh   # force a fresh token acquisition
python main.py teams-pipeline list                  # recent meeting jobs
python main.py teams-pipeline list --status failed  # only failed jobs
python main.py teams-pipeline show <job-id>         # full detail of one job
python main.py teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
python main.py teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
python main.py teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
python main.py teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
python main.py teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

python main.py teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
python main.py teams-pipeline delete-subscription <sub-id>
python main.py teams-pipeline maintain-subscriptions            # renew near-expiry ones
python main.py teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `python main.py teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `python main.py cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
