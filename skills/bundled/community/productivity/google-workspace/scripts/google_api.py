#!/usr/bin/env python3
"""Google Workspace API CLI for VoiceOS.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing VoiceOS-facing JSON contract and falls back to the Python client
...
from _voiceos_home import get_voiceos_home
...
def _gws_binary() -> str | None:
    override = os.getenv("VOICEOS_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for VoiceOS")
