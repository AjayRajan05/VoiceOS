...
metadata:
  voiceos:
    ...
    related_skills: [claude-code, voiceos-platform]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the VoiceOS terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For VoiceOS itself, `model.provider: openai-codex` uses VoiceOS-managed Codex
OAuth from `~/.voiceos/auth.json` after `python main.py auth add openai-codex`. For the
...
## VoiceOS Gateway Caveat

When invoking the Codex CLI from a VoiceOS gateway/service context (for example,
